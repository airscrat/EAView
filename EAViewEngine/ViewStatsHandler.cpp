...
{
	ViewStatsHandler::ViewStatsHandler(void):
		_keyEventTogglesOnScreenStats('c'),
		_keyEventPrintsOutStats('C'),
		_statsType(NO_STATS),
		_initialized(false),
	...
	ViewStatsHandler::~ViewStatsHandler()
	{
	}
	bool ViewStatsHandler::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
	{
						...
						case VIEWER_STATS:
							{
								...
								osgViewer::ViewerBase::Scenes::iterator itr = scenes.begin();
	...
	void ViewStatsHandler::updateThreadingModelText()
	{
		switch(_threadModel)
		{
		case(osgViewer::Viewer::SingleThreaded): _threadModelText->setText("ThreadingModel: SingleThreaded"); break;
		case(osgViewer::Viewer::CullDrawThreadPerContext): _threadModelText->setText("ThreadingModel: CullDrawThreadPerContext"); break;
		case(osgViewer::Viewer::DrawThreadPerContext): _threadModelText->setText("ThreadingModel: DrawThreadPerContext"); break;
		case(osgViewer::Viewer::CullThreadPerCameraDrawThreadPerContext): _threadModelText->setText("ThreadingModel: CullThreadPerCameraDrawThreadPerContext"); break;
		case(osgViewer::Viewer::AutomaticSelection): _threadModelText->setText("ThreadingModel: AutomaticSelection"); break;
		default:
			_threadModelText->setText("ThreadingModel: unknown"); break;
		}
	}
	void ViewStatsHandler::reset()
	{
		_initialized = false;
		_camera->setGraphicsContext(0);
		_camera->removeChildren( 0, _camera->getNumChildren() );
	}
	void ViewStatsHandler::addUserStatsLine(const std::string& label, const osg::Vec4& textColor, const osg::Vec4& barColor,
	...
	void ViewStatsHandler::removeUserStatsLine(const std::string& label)
	{
	...
	void ViewStatsHandler::setUpHUDCamera(osgViewer::ViewerBase* viewer)
	{
	...
	void ViewStatsHandler::setWindowSize(int width, int height)
	{
		if (width <= 0 || height <= 0)
			return;
	...
	osg::Geometry* ViewStatsHandler::createBackgroundRectangle(const osg::Vec3& pos, const float width, const float height, osg::Vec4& color)
	{
	...
	osg::Geometry* ViewStatsHandler::createGeometry(const osg::Vec3& pos, float height, const osg::Vec4& colour, unsigned int numBlocks)
	{
		osg::Geometry* geometry = new osg::Geometry;
		geometry->setUseDisplayList(false);
		osg::Vec3Array* vertices = new osg::Vec3Array;
	...
	struct FrameMarkerDrawCallback : osg::Drawable::DrawCallback
	{
	...
	osg::Geometry* ViewStatsHandler::createFrameMarkers(const osg::Vec3& pos, float height, const osg::Vec4& colour, unsigned int numBlocks)
	{
		osg::Geometry* geometry = new osg::Geometry;
		geometry->setUseDisplayList(false);
		osg::Vec3Array* vertices = new osg::Vec3Array;
	...
	osg::Geometry* ViewStatsHandler::createTick(const osg::Vec3& pos, float height, const osg::Vec4& colour, unsigned int numTicks)
	{
		osg::Geometry* geometry = new osg::Geometry;
		geometry->setUseDisplayList(false);
		osg::Vec3Array* vertices = new osg::Vec3Array;
	...
	void ViewStatsHandler::setUpScene(osgViewer::ViewerBase* viewer)
	{
			...
			++citr)
		{
			if ((*citr)->getGraphicsContext())
			{
				...
				/*const osg::GLExtensions* extensions = state->get<osg::GLExtensions>();state->getg
				if (extensions &&
			...
			*/}
				...
				++itr)
			{
				...
				if (dp && dp->isRunning())
				{
					...
					pos.x() = averageLabel->getBound().zMax();
		...
		// Camera scene stats
		{
			...
			std::wstring wstr;
			CChineseCode::StringToWString(viewStr.str(),wstr);
			camStaticText->setText(wstr.c_str());
	...
	void ViewStatsHandler::createTimeStatsLine(const std::string& lineLabel,
	...
	void ViewStatsHandler::createCameraTimeStats(osg::Vec3& pos, bool acquireGPUStats, osg::Stats* viewerStats, osg::Camera* camera)
	{
	...
	void ViewStatsHandler::getUsage(osg::ApplicationUsage& usage) const
	{
